...
description: Fixes the Checkmarx One findings Triage Assist has confirmed on the current branch, with fixes generated by Checkmarx Remediation Assist, applied as edits the developer reviews and can undo. Use when the developer asks to find, list, check, fix, remediate, or clean up Checkmarx findings, security findings, vulnerabilities, or scan results in this project.
model: inherit
...
3. `python3 "$FTF" stage` computes fixes without touching the workspace.
   ...
   with the Edit tool; the developer reviews each change afterwards. Never write files
