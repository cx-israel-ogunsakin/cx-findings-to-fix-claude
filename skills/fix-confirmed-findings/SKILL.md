...
description: Find and fix the Checkmarx One security findings that Triage Assist has confirmed on this repo's branch, with fixes generated by Checkmarx Remediation Assist and applied as edits you accept or reject. Use whenever the developer mentions Checkmarx, security findings, vulnerabilities, scan results, SAST, or asks to find, list, show, check, fix, remediate, patch, or clean up security issues in this project, even if they do not name Checkmarx.
---
...
`patched_path` with the file-reading tool and apply it; the editor's diff
...
3. **Present.** From the manifest JSON, show a compact table of `results`:
   ...
   and stop.
4. **Confirm scope.** Ask which findings to fix. Default is all with status READY; each file still
   arrives as its own reviewable edit.
5. **Compute the fixes.** Run `stage` (optionally `--only 0,2`) from the same workspace
...
6. **Apply each ready fix as an editor edit** (read `patched_path`, apply it
   as the new content of `file`), one file at a time; the developer gets a
   diff to accept or reject. Before the first edit, say in one line that each
   fix is being applied and will appear as a diff to accept or reject, and
   that if changes land without that diff, auto-accept is on and can be
   turned off. Never write files with a terminal
...
7. **Drifted files (`needs_assist`):** read `patch_path` and the local file.
   ...
   first, then apply the same change where the code lives now as an editor
...
8. **Test files are platform files; never author tests.** The platform's
   ...
   under `tests`); apply or place them like any other file. If one cannot be
