...
description: Find and fix the Checkmarx One security findings that Triage Assist has confirmed on this repo's branch, with fixes generated by Checkmarx Remediation Assist and applied as edits you review and can undo. Use whenever the developer mentions Checkmarx, security findings, vulnerabilities, scan results, SAST, or asks to find, list, show, check, fix, remediate, patch, or clean up security issues in this project, even if they do not name Checkmarx.
---
...
6. **Apply each ready fix as an editor edit** (read `patched_path`, apply it
   as the new content of `file`), one file at a time. Before the
   first edit, say in one line that the fixes are being applied now and can
   be reviewed and undone afterwards. Never write files with a terminal
