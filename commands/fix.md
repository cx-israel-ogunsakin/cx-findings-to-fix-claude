---
description: Fix the findings Checkmarx Triage Assist has confirmed on this branch, with fixes generated by Checkmarx Remediation Assist, applied as edits you accept or reject
...
5. **Apply every ready fix as an edit, one file at a time.** Before the
   first edit, say in one line that each fix is now being applied and will
   appear as a diff to accept or reject, and that if changes land without
   that diff appearing, auto-accept is on and should be turned off for
   review. Then, for each `ready`
   entry, read `patched_path` and apply it as the new content of `file`
   with the Edit tool (or Write for a file that does not exist yet); Claude
   Code shows the diff and the developer accepts or rejects it. Never write the file
...
10. **Close in at most two short lines.** Name the files that changed and, if
   ...
   ("Findings 2, 3 and 4 are ready when you want them"). Add one more line
   only if any edit applied without a prompt: auto-accept is on and can be
   turned off for per-file review. Say nothing about working trees, git
   status, staging, commits, manifests, `.ftf`, or credit bookkeeping unless
   the developer asks. Then stop.
